## **Proyecto-Mission Control.**

## Información del Producto.

**Nombre del Producto:** *Mission Control.*

**Descripción:** *Aplicación dedicada a la gestión de inventario.*

## Integrantes del Equipo

* *Barraza Chavez Diego Alejandro*
* *Calleros Valdenegro José Adrián*
* *Inzunza Gómez Jesús Daniel*
* *Urias Ruiz Ernesto*

# **1. Introducción.**  
  
**Descripción general:** Mission Control Es una plataforma digital de gestión de inventario inteligente orientada a el manejo de documentacion dentro de las empresas como lo es Passione (compañia ficticia). Su objetivo principal es eliminar el error humano y eliminar la ineficiencia de la memoria humana y poder llevar un registro exacto mediante una API y una base de datos.
  
**Audiencia:** Clientes finales, administradores del sitio, equipo de desarrollo y soporte técnico.  
  
**Cobertura:** 
Para el desarrollo y buen funcionamiento de *NovaHardware* nos apoyamos de muy buenas herramientas y recursos académicos.
 
*Recursos Tecnológicos*

- [Node.js](https://nodejs.org/) 
- [Express.js](https://expressjs.com/) 
- [PostgreSQL](https://www.postgresql.org/)
- [JavaScript](https://developer.mozilla.org/es/docs/Web/JavaScript)

**Herramientas de Desarrollo**

- [Visual Studio Code](https://code.visualstudio.com/)
- [GitHub (Repositorio InventarioRegistro)](https://github.com/MONIGIR/InventarioRegistro)
- [PostgreSQL](https://www.postgresql.org/)
- 
**Prototipado y Diseño**

- [Figma - Prototipo Mission Control]([----](https://www.figma.com/design/uLzO2IP2zJevEALUE92yQE/Mission-Control?node-id=3-2&t=qeB6uMfcU9GhjFsG-1))

